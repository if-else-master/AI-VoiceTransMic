...
class VoiceTranslationGUI:
    def __init__(self):
        # 創建主窗口
        self.root = tk.Tk()
        self.root.title("🎤 即時語音克隆翻譯系統 (OpenVoice)")
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(title_frame, text="🎤 即時語音克隆翻譯系統 (OpenVoice)", 
                              font=('SF Pro Display', 24, 'bold'), 
                              bg=self.colors['bg_primary'], 
                              fg=self.colors['text_primary'])
        title_label.pack()
        
        subtitle_label = tk.Label(title_frame, text="Deep Simultaneous Translation with OpenVoice", 
        ...
        # 語音克隆區域
        voice_frame = tk.LabelFrame(self.root, text="🎭 語音克隆 (OpenVoice)", 
                                   font=('SF Pro Display', 14, 'bold'), 
                                   bg=self.colors['bg_secondary'], 
                                   fg=self.colors['text_primary'],
                                   bd=2, relief='groove',
        ...
        self.load_model_btn = tk.Button(voice_frame, text="🤖 載入模型", command=self.load_openvoice_model,
                                       bg=self.colors['button_bg'], fg='black', 
                                       font=('SF Pro Display', 11, 'bold'),
                                       bd=0, relief='flat', cursor='hand2',
                                       padx=15, pady=8)
        ...
        tk.Label(translated_label_frame, text="Real-time Translation (OpenVoice)", 
                bg=self.colors['bg_primary'], 
                fg=self.colors['text_secondary'],
                font=('SF Pro Display', 10)).pack(side='right')
    ...
    def load_existing_voices(self):
        ...
        try:
            ...
            if voice_names:
                ...
                # 如果已載入模型，嘗試提取語音特徵
                if self.backend.tone_color_converter and not self.backend.target_se:
                    try:
                        self.backend.target_se = self.backend.tone_color_converter.extract_se(self.backend.cloned_voice_path)
                        print("✅ 語音特徵提取完成")
                    except Exception as e:
                        print(f"⚠️ 語音特徵提取失敗: {e}")
    ...
    def load_openvoice_model(self):
        """載入 OpenVoice 模型"""
        try:
            self.update_status("🔄 載入 OpenVoice 模型...")
    ...
    def _load_model_thread(self):
        """模型載入線程"""
        try:
            success = self.backend.load_openvoice_models()
    ...
    def _model_loaded_callback(self, success, error=None):
        """模型載入完成回調"""
        if success:
            self.update_status("✅ OpenVoice 模型載入成功！")
    ...
    def start_translation(self):
        ...
        if not self.backend.tone_color_converter:
            messagebox.showerror("錯誤", "請先載入 OpenVoice 模型")
            return
...
class RealTimeVoiceTranslationSystem:
    def __init__(self):
        ...
        print("🎤 即時語音克隆翻譯系統已啟動！(OpenVoice版本)")
    
    def load_openvoice_models(self):
        """載入OpenVoice模型"""
        try:
            ...
            # 如果已有語音文件但尚未提取特徵，則提取特徵
            if self.cloned_voice_path and not self.target_se:
                try:
            ...
            print("✅ OpenVoice ToneColorConverter 載入成功！")
            return True
            
        except Exception as e:
            print(f"❌ 載入 OpenVoice 模型失敗: {e}")
            return False
    
    def get_tts_model(self, language):
        """獲取指定語言的TTS模型"""
        if language not in self.tts_models:
            try:
                openvoice_lang = self.openvoice_language_map.get(language, 'EN_NEWEST')
                self.tts_models[language] = TTS(language=openvoice_lang, device=self.device)
                print(f"✅ 載入 {openvoice_lang} TTS 模型成功")
            except Exception as e:
                print(f"❌ 載入 {language} TTS 模型失敗: {e}")
                return None
    ...
    def clone_voice_step_gui(self):
        ...
        try:
            ...
            if self.tone_color_converter:
                ...
                print("⚠️ ToneColorConverter 尚未初始化，將在載入模型時提取語音特徵")
            ...
            try:
                ...
                if translated_text and translated_text.strip():
                    ...
                    if self.gui and self.gui.enable_voice_var.get():
                        output_file = self.synthesize_speech_openvoice(translated_text)
                        if output_file:
    ...
    def synthesize_speech_openvoice(self, text):
        """使用OpenVoice合成語音"""
        try:
            if not self.tone_color_converter or not self.target_se:
                print("❌ OpenVoice模型或語音特徵未準備好")
                return None
            
            # 獲取目標語言的TTS模型
            tts_model = self.get_tts_model(self.target_language)
            if not tts_model:
                print(f"❌ 無法載入 {self.target_language} TTS 模型")
                return None
            
            # 生成基礎語音
            src_path = f'{self.output_dir}/tmp.wav'
            speaker_ids = tts_model.hps.data.spk2id
            speaker_key = list(speaker_ids.keys())[0]  # 使用第一個可用的speaker
            ...
            if os.path.exists(source_se_path):
                ...
                print(f"⚠️ 找不到source speaker特徵文件: {source_se_path}")
                return src_path  # 返回原始語音
            ...
            print(f"🔊 OpenVoice語音合成完成")
            return output_file.name
            
        except Exception as e:
            print(f"❌ OpenVoice語音合成錯誤: {e}")
            return None
